...
class TestModeSwitching(unittest.TestCase):
    ...
    def test_completion_after_mode_switch_to_dos(self):
        ...
        document = Document("@m")
        completions = list(self.handler.completer.get_completions(document, CompleteEvent()))
    ...
    def test_completion_after_mode_switch_to_basic(self):
        ...
        document = Document("@")
        completions = list(self.handler.completer.get_completions(document, CompleteEvent()))
        self.assertGreater(len(completions), 1, "BASICモードでは複数の@コマンドがあるはず")
    ...
    def test_multiple_mode_switches(self):
        ...
        self.assertGreater(basic_command_count, 0, "BASICモードでPコマンドが補完されるはず")
